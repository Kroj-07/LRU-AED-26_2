...
//Secuencia 5: eviction didáctica (MRU -> LRU, como se expulsa LRU)
void secuenciaEvictionDidactica() {
    std::cout << "\n Secuencia 5: eviction didáctico \n";
    TraceLogger logger("trace_eviction_didactica.jsonl");
    LRUCache cache(3, &logger);

    cache.put(1, 10);
    cache.put(2, 20);  
    cache.put(3, 30); // caché llena: [3, 2, 1]
    cache.get(1); // hit: [1, 3, 2]
    cache.put(4, 40); // evict 2: [4, 1, 3]
    cache.get(2); // miss 2 notin cache
    cache.get(3); // hit: [3, 4, 1]
...
int main() {
    ...
    secuenciaEvictionDidactica();
    std::cout << "\nListo. 5 archivos trace.jsonl generados.\n";
    return 0;
}
